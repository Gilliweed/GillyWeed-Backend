...
def clean_answer(answer):
    answer=answer.replace(' ','').replace('Rs.','')
...
def calculate_footprint(answers):
    ...
    # Rs 9.87/kwh/0.62 kg CO2/kwh = Rs 15.92/kg CO2  - all we need to do is divide montly bill by this.
    # electric bill = (electric bill / people in household) / (Rs 15.92/kgCo2)     
    
    try:
        answer_1=answer_1.replace('$','')
        electric_=(int(answer_2)/int(answer_1))*12/15.92
    except:
    ...
    answer_11=answer_11.replace('Rs','').replace(' ','')
    retail_=4*float(answer_11)
...
def make_graphs(individual_means, individual_means_2):
    ...
    population_means = [7252.76, 602.45, 4515.27, 2267.96, 1025.41]
    population_means=list(map(int,population_means))
...
def make_bar_pdf(pdfname, logo):
    ...
    c=canvas.Canvas(pdfname, pagesize=portrait(letter))
    c.setFont('Helvetica-Bold', 16, leading=None)
    c.drawCentredString(300,600,"Your carbon consumption relative to the average Person (units)")
    ...
    c.drawCentredString(300,50,"Gillyweed")
...
def make_bar_2_pdf(pdfname, logo, footprint_delta):
    ...
    c=canvas.Canvas(pdfname, pagesize=portrait(letter))
    c.setFont('Helvetica-Bold', 16, leading=None)
    c.drawCentredString(300,600,"Your carbon consumption relative to the average Person (kg CO2)")
    c.drawImage(logo, 0, 250, width=600,height=300, preserveAspectRatio=True)

    # now draw the delta 
    c.setFont('Helvetica', 11, leading=None)
    if float(footprint_delta) <= 5500:
        c.drawCentredString(300, 200, "Your carbon consumption is %s kg CO2 less than the average Person."%(footprint_delta))
    else:
        c.drawCentredString(300, 200, "Your carbon consumption is %s kg CO2 over the average Person."%(footprint_delta))
...
def improvement_pdf(pdfname, truthlist):
    ...
    recommendation_list=['ask questions to learn more about RECYCLED AND SUSTAINABLE PRODUCTS',
                       ...
                       'YOU CAN ALSO CHECK OUR WEBSITE FOR AFFORDABLE SUSTAINABLE AND RECYCLED PRODUCTS']
...
# example = 50 
answer_2 = input('What is your electric bill (in Rs.) monthly?  (e.g. 50) \n')
...
# example = yes 
answer_7 = input('Do you use uber or another ride sharing platform like Ola? (e.g. y) \n')
...
# example = 50
answer_11 = input('How much money do you spend on Amazon per month in Rs. - for example, 9000? (e.g. 9000) \n')
